...
function ChatBox() {
  ...
  useEffect(() => {
    ...
    .then((res) => res.json())
    .then((data) => {
      setMessages(data.messages.sort((a:any,b:any)=>b.createdAt > a.createdAt));
    ...
    <div className="h-screen flex-column justify-center items-center px-40">
      ...
      {loading ? ( // Render a loading indicator while data is loading
        ...
        <>
          ...
          <hr></hr>
            <input name="message" value={typedMessage} type="text" placeholder="Type your message here" onChange={(e)=>setTypedMessage(e.target.value)}></input>
            <button className="btn btn-primary" onClick={sendMessage}>Send</button>
          <div className="message-container">
            {messages.map((message, index) => (
              <div
                key={index}
                className={`message ${message.sender === myID ? "my-message" : "other-message"}`}
              >
  ...
  function initiateSocketConnection(myID: string, otherPersonID: string, chatboxID: string) {
